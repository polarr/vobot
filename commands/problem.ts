...
import { CommandInteraction, MessageEmbed, MessageAttachment, MessageActionRow, MessageButton } from 'discord.js';
import { Exams, Problem, randomInt, problemDifficulty, tests, scrapeProblem } from '../scrape.js';
import { webkit, ElementHandle } from 'playwright';
...
export default {
    ...
    async execute(interaction: CommandInteraction) {
        ...
        if (options.getSubcommand() === 'random') {
            let t = Object.keys(tests)[randomInt(0, Object.keys(tests).length - 1)] as Exams;
            let test = tests[t];
            ...
            const [[contest, modifier], year, problem] = [(options.getString("contest") as string).split(' ') as [Exams, string], options.getInteger("year") as number, options.getInteger("problem") as number];
            ...
            let [t, after_year, before_year, after_problem, before_problem] = [options.getString('contest') as Exams, options.getInteger('after_year'), options.getInteger('before_year'), options.getInteger('after_problem'), options.getInteger('before_problem')];
        ...
        const problemEl = await page.$('.problem') as ElementHandle<HTMLElement>;
        ...
        const boundingBox = await problemEl.boundingBox() as BoundingBox;

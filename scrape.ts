...
type Exams = "AMC 8" | "AMC 10" | "AMC 12" | "AIME";
type ExamTypes = "choice" | "answer" | "proof";
...
const tests: {[name in Exams]: Test} = {
    "AMC 8": {
        name: "AMC 8",
        urlName: "AMC_8",
        type: 'choice',
...
function lerp(x1: number, x2: number, y1: number, y2: number, a: number){
    return y1 + (a - x1) * (y2 - y1)/(x2 - x1);
}

function problemDifficulty(t: Exams, year: number, problem: number) {
    let test: Test = tests[t];
    let diff: number = 0;
...
async function scrapeProblem({test, modifier, year, problem, difficulty}: Problem) {
    const url: string = `https://artofproblemsolving.com/wiki/index.php/${year}_${tests[test].urlName}${modifier}`;
    try {
...
export {Exams, Problem, randomInt, problemDifficulty, tests, scrapeProblem};
